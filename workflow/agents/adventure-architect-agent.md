...
### 5. Produce the adventure JSON skeleton

Produce a valid adventure JSON file containing:

- `id`, `title`, `description`, `startSection`
- `items` — full item definitions for all items identified in the design brief and gate contracts. Chapter-specific items that are not cross-chapter may be left as stubs (`{ "name": "...", "description": "..." }`) to be filled by chapter authors.
...
## Ready for Authoring
Chapters that can be authored immediately (no manifest dependencies on unwritten chapters):
- ch1, ch3 (no incoming cross-chapter items)

Chapters that must wait:
- ch2 — depends on items introduced in ch1
- ch4 — depends on state set in ch3
```
...
| Items defined | Every item named in gate contracts has a definition in the adventure `items` list |
