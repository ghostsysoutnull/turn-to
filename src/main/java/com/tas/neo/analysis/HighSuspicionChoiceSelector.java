...
/**
 * Prefers choices whose target section is in the suspicion-raising set (weight 3 vs 1).
 */
public class HighSuspicionChoiceSelector implements ChoiceSelector {

    private final Set<Integer> suspicionSections;

    public HighSuspicionChoiceSelector(Set<Integer> suspicionSections) {
    ...
    public int select(List<Choice> choices, SimulatedGameState state, Random random) {
        ...
        for (int i = 0; i < choices.size(); i++) {
            int weight = isSuspicionRaising(choices.get(i)) ? 3 : 1;
            weights[i] = weight;
            totalWeight += weight;
        }
